function createVehicleForPlayer(player, command, model)
    ...
    dbExec(db, 'INSERT INTO vehicles (model, x, y, z, rotation_x, rotation_y, rotation_z) VALUES (?, ?, ?, ?, ?, ?, ?)',
...
function loadAllVehicles(queryHandle)
    ...
    for index, vehicle in pairs(results) do
        local vehicleObject = createVehicle(vehicle.model, vehicle.x, vehicle.y, vehicle.z, vehicle.rotation_x,
            vehicle.rotation_y, vehicle.rotation_z)
...
addEventHandler('onResourceStop', resourceRoot, function()
    ...
    for index, vehicle in pairs(vehicles) do
        ...
        dbExec(db,
            'UPDATE vehicles SET x = ?, y = ?, z = ?, rotation_x = ?, rotation_y = ?, rotation_z = ? WHERE id = ?', x,
            y, z, rotX, rotY, rotZ, id)
    end
end)
